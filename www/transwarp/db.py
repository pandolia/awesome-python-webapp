...
def connection():
    '''
    Reture _ConnectionCtx object that can be used by 'with' statement:
    with connection():
        # do something here
        pass
    '''
...
class _TransactionCtx(object):
    ...
    def __enter__(self):
        ...
        logging.info('begin transaction...' if _db_ctx.transactions == 1 \
                            else 'join current transaction...')
        return self
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    params = dict(user=user, password=password, database=database, 
                                            host=host, port=port)
    defaults = dict(use_unicode=True, charset='utf8', collation='utf8_general_ci',
                        autocommit=False)
...
def select_one(sql, *args):
    ...
    >>> u1 = dict(id=100, name='Alice', email='alice@test.org', passwd='ABC-12345',
    ...         last_modified=time.time())
    >>> u2 = dict(id=101, name='Sarah', email='sarah@test.org', passwd='ABC-12345',
    ...         last_modified=time.time())
    >>> insert('user', **u1)
    1
    >>> insert('user', **u2)
...
def update(sql, *args):
    '''
    Execute update SQL.
    >>> u1 = dict(id=1000, name='Michael', email='michael@test.org', passwd='123456',
    ...                                     last_modified=time.time())
    >>> insert('user', **u1)
    1
...
def main():
    ...
    create_engine(user='test_admin', password='123', database='test')
...
if __name__ == '__main__':
    ...
    # all_users = select('select * from user')
    ...
    # fmt = '%-20s' * len(all_users[0])

    # keys = ('id', 'name', 'email', 'passwd', 'last_modified')
    
    # print fmt % keys

    # for user in all_users:
    #     print fmt % tuple([user[key] for key in keys])
