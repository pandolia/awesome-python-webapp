    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.iteritems():
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    if primary_key:
                        raise TypeError('Cannot define more than 1 primary key '
                                        'in class: %s' % name)
                    if v.updatable:
...
class Model(dict):
    ...
    def count_all(cls):
        '''
        'select count(pk) from table' and return integer.
        '''
    ...
    def count_by(cls, where, *args):
        '''
        'select count(pk) from table where ... ' and return integer.
        '''
        return db.select_int('select count(`%s`) from `%s` %s' % \
                             (cls.__primary_key__.name, cls.__table__, where), \
                             *args)
