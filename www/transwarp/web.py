...
class Response(object):
    ...
    def content_type(self, value):
        '''
        Set content type for response. This is a shorcut for set_header('Content-Type', value).
        '''
        if value:
    ...
    def set_cookie(self, name, value, max_age=None, expires=None, path='/', domain=None, secure=False, http_only=True):
        ...
        Args:
          ...
          expires: optional, unix timestamp, datetime or date object that indicate an absolute time of the expiration time of cookie. Note that if expires specified, the max_age will be ignored.
          path: the cookie path, default to '/'.
          domain: the cookie domain, default to None.
          secure: if the cookie secure, default to False.
          http_only: if the cookie is for http only, default to True for better safty (client-side script cannot access cookies with HttpOnly flag).
        >>> r = Response()
...
def default_error_handler(e, start_response, is_debug):
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                fp.close()
                start_response('500 Internal Server Error', [])
                return [ r'''<html><body><h1>500 Internal Server Error</h1><div style="font-family:Monaco, Menlo, Consolas, 'Courier New', monospace;"><pre>''', stacks.replace('<', '&lt;').replace('>', '&gt'), '</pre></div></body></html']
            finally:
                del ctx.application
                del ctx.request
                del ctx.response
